...
export default function ClubForm({ initial }: { initial: Club | null }) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      {/* A club with a detail page is edited under About Pages, not
          here — this form stays the card row so the thirteen card-only
          clubs keep a short screen. */}
      {initial?.introHeading && (
        <p className="text-sm text-gray-600">
          This club has its own page. Edit its hero, intro, figures,
          activities, and closing panel under{' '}
          <Link href={`/admin/about-club/${initial.slug}`}
                className="font-medium text-accent hover:underline">
            About Pages → {initial.name}
          </Link>
          .
        </p>
      )}
